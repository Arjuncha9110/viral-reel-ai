...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Navigation is handled in AuthContext, but we can also do it here if preferred.
    } catch (err: any) {
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-3xl shadow-glow-gold relative overflow-hidden">
        ...
        <div>
          ...
          <p className="mt-2 text-center text-sm text-slate-600">
            Sign in to access your spiritual guidance
          </p>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center px-4 py-3 border border-slate-300 rounded-xl shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm">
              <Link to="/forgot-password" className="font-medium text-amber-600 hover:text-amber-500 transition-colors">
