import { PlanetPosition } from "./kundaliEngine";
 ...
 */
export const calculateDivisionalSign = (longitude: number, division: number, chartType: ChartType = "D1"): number => {
    // Normalize longitude
...
const normalizeSign = (val: number): number => {
    let s = val % 12;
    return s === 0 ? 12 : s;
};
 ...
 */
export const getVargaPlanets = (planets: PlanetPosition[], chartType: ChartType): PlanetPosition[] => {
