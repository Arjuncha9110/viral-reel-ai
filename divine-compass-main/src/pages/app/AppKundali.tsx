...
export const AppKundali: React.FC = () => {
  ...
  const localChartData = useMemo(() => {
    if (!profile?.birthDetails || !profile.birthDetails.date || !profile.birthDetails.time) return null;
    try {
      const { date, time, latitude, longitude, timezoneId } = profile.birthDetails;
  ...
  if (!profile || !profile.birthDetails) {
    return (
      <AppShell title="Janam Kundali" eyebrow="Your Birth Chart" showBack>
  ...
  const personName = profile.profile?.displayName || currentUser?.displayName || undefined;
