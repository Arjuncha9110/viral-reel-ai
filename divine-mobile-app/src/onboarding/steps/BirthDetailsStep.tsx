...
export const BirthDetailsStep: React.FC<BirthDetailsStepProps> = ({ initialData, onNext, onBack }) => {
  ...
  const handleNext = async () => {
    ...
    if (placeOfBirth.trim().length < 3) {
      return setError("Please enter the full birth city name, for example Bengaluru, Delhi, or Mumbai.");
    }
    ...
    if (!loc) {
      return setError("I could not find that birth place. Please select a city from the dropdown, or enter the full city and country.");
    }
  ...
  const canProceed = !isLoading && !!date && !!time && placeOfBirth.trim().length >= 3;
